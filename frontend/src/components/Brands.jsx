...
/* =========================================
 * ⚡ BRANDS SECTION COMPONENT (THEME INTEGRATED) ⚡
 * ========================================= */
export default function BrandsSection() {
  return (
    <section 
      style={{ backgroundColor: 'var(--theme-bg-light)' }} 
      className="py-16 sm:py-24 selection:bg-magic-primary selection:text-black transition-colors duration-500"
    >
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
        
        {/* ⚡ Centered Heading: Brands ⚡ */}
        <div className="flex flex-col items-center justify-center mb-6 text-center">
          <h2 className="text-3xl font-bold leading-none" style={{ color: 'var(--theme-text-main)', fontFamily: "'Inter', sans-serif" }}>
            Brands
          </h2>
        </div>

        {/* ⚡ LOGO GRID (Layout untouched) ⚡ */}
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-y-12 sm:gap-y-3 gap-x-8 lg:gap-x-12 px-4 lg:px-12 max-w-[1400px] mx-auto items-center justify-items-center">
          {brands.map((brand) => (
            <div key={brand.id} className="w-full flex items-center justify-center h-20 hover:scale-105 transition-transform duration-300 cursor-pointer grayscale hover:grayscale-0 opacity-80 hover:opacity-100">
              ...
              {brand.name === "tru=e" ? (
                <div className="flex flex-col items-center justify-center mt-2">
                   <div className="flex gap-[3px] items-end mb-1">
                     <div className="w-[2px] h-5" style={{ backgroundColor: 'var(--theme-text-main)' }}></div>
                     <div className="w-[3px] h-8" style={{ backgroundColor: 'var(--theme-text-main)' }}></div>
                     <div className="w-[3px] h-10" style={{ backgroundColor: 'var(--theme-text-main)' }}></div>
                     <div className="w-[3px] h-8" style={{ backgroundColor: 'var(--theme-text-main)' }}></div>
                     <div className="w-[2px] h-5" style={{ backgroundColor: 'var(--theme-text-main)' }}></div>
                   </div>
                   <div className="w-10 h-[2px] mb-1" style={{ backgroundColor: 'var(--theme-text-main)' }}></div>
                   <p className="text-lg font-light tracking-[0.2em] mt-1 font-sans" style={{ color: 'var(--theme-text-main)' }}>tru<span className="font-bold">=</span>e</p>
                </div>
              ) : brand.name === "JLab" ? (
                <div className="flex items-center gap-1.5">
                   <div className="w-7 h-7 rounded-full border-[3px] flex items-center justify-center" style={{ borderColor: 'var(--theme-primary)' }}>
                     <div className="w-2.5 h-2.5 rounded-sm rotate-45" style={{ backgroundColor: 'var(--theme-primary)' }}></div>
                   </div>
                   <span className="font-sans font-black text-2xl tracking-tighter uppercase" style={{ color: 'var(--theme-text-main)' }}>JLAB</span>
                </div>
              ) : brand.name === "Ledger" ? (
                <div className="flex items-center gap-2">
                   <div className="grid grid-cols-2 gap-[2px]">
                      <div className="w-3 h-3 rounded-[1px]" style={{ backgroundColor: 'var(--theme-text-main)' }}></div>
                      <div className="w-3 h-3 rounded-[1px]" style={{ backgroundColor: 'var(--theme-text-main)' }}></div>
                      <div className="w-3 h-3 rounded-[1px]" style={{ backgroundColor: 'var(--theme-text-main)' }}></div>
                      <div className="w-3 h-3 border-[2px] rounded-[1px]" style={{ borderColor: 'var(--theme-text-main)' }}></div>
                   </div>
                   <span className="font-sans font-bold text-2xl tracking-tight" style={{ color: 'var(--theme-text-main)' }}>Ledger</span>
                </div>
              ) : brand.name === "Goldmedal" ? (
                <div className="flex flex-col items-center">
                   <div className="flex items-center gap-1.5">
                      <div className="w-5 h-5 border-[2px] rounded-sm flex items-center justify-center relative" style={{ borderColor: 'var(--theme-text-main)' }}>
                        <div className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: 'var(--theme-text-main)' }}></div>
                      </div>
                      <span className="font-serif font-black text-[22px] tracking-tighter" style={{ color: 'var(--theme-primary)' }}>Goldmedal</span>
                   </div>
                   <span className="text-[6px] font-sans tracking-[0.25em] uppercase font-bold mt-1 ml-4" style={{ color: 'var(--theme-text-main)' }}>Switches & Systems</span>
                </div>
              ) : brand.name === "nanoleaf" ? (
                <div className="flex flex-col items-center">
                   <div className="flex items-center gap-1">
                      <div className="w-6 h-6 rounded-tl-full rounded-br-full rounded-tr-md rounded-bl-md relative flex items-center justify-center" style={{ backgroundColor: 'var(--theme-primary)' }}>
                        <div className="w-1.5 h-1.5 bg-white rounded-full absolute top-1 right-1"></div>
                      </div>
                      <span className="font-sans font-bold text-[26px] tracking-tighter lowercase" style={{ color: 'var(--theme-text-main)' }}>nanoleaf</span>
                   </div>
                   <span className="text-[6px] font-sans tracking-widest mt-0.5 ml-6" style={{ color: 'var(--theme-primary)' }}>Smarter by Design</span>
                </div>
              ) : (
                /* ======== STANDARD TEXT LOGOS ======== */
                <div className="w-full h-full flex items-center justify-center text-center">
                  <span className={`${brand.className}`} style={{ color: 'var(--theme-text-main)' }}>
                    {brand.name}
                  </span>
                </div>
              )}
