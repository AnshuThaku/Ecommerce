...
const SearchBar = ({ onSearchSuccess }) => (
  <div className="relative w-full">
    <input 
      type="text" 
      placeholder="Search for luxury products..." 
      className="w-full bg-zinc-900 border border-zinc-800 rounded-full px-6 py-4 text-white focus:outline-none focus:ring-1 focus:ring-[var(--theme-primary)] transition-all" 
...
export default function Header() {
  ...
  const SidebarItem = ({ icon: Icon, label, to, isActive = false }) => (
    <Link 
      to={to} 
      className={`relative flex items-center justify-between px-6 py-3 text-[12px] font-semibold transition-all group/item
        ${isActive ? 'bg-zinc-50 text-black border-r-4 border-black' : 'text-zinc-700 hover:bg-zinc-50 hover:text-black'}
      `}
    >
      <div className="flex items-center gap-4">
        <Icon size={18} className={isActive ? "text-black" : "text-zinc-500 group-hover/item:text-black"} />
  ...
  return (
    <div className="font-['Montserrat']">
      ...
      {/* ⚡ THEME UPDATE: Exact wahi classes, sirf variables badle hain */}
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Montserrat:wght@300;400;500;600;700&display=swap');
        ...
        .promo-bar { 
          ...
          background: var(--theme-bg-dark); color: var(--theme-text-light); font-size: 9px; font-weight: 600; 
        ...
        .navbar { 
          ...
          display: flex; align-items: center; justify-content: space-between; padding: 8px 80px; 
          background: var(--theme-bg-dark); height: 70px; border-bottom: 1px solid rgba(255,255,255,0.05); 
          transition: background-color 0.5s ease;
        }
        
        .nav-center { display: flex; align-items: center; gap: 30px; position: absolute; left: 50%; transform: translateX(-50%); }
        .nav-link { color: #999; text-decoration: none; font-size: 11px; font-weight: 600; letter-spacing: 0.18em; text-transform: uppercase; transition: color 0.2s; display: flex; align-items: center; gap: 4px; }
        .nav-link:hover { color: var(--theme-text-light); }
        
        .cart-badge { 
          position: absolute; top: -7px; right: -8px; 
          background: var(--theme-primary); color: var(--theme-text-main); font-size: 8px; font-weight: 800; 
        ...
        .search-overlay {
          ...
          z-index: 600; display: flex; flex-direction: column; align-items: center; padding-top: 100px;
        }
      ...
      {isSearchOpen && (
        <div className="search-overlay">
          ...
          <div className="w-full max-w-3xl px-6">
            <h2 className="text-[var(--theme-primary)] font-serif text-2xl mb-8 text-center italic tracking-wider transition-colors duration-500">
      ...
      {/* ⚡ THEME UPDATE: Exact same div, sirf var(--theme-nav) ki jagah var(--theme-bg-dark) */}
      {isMobileMenuOpen && (
        <div className="fixed inset-0 z-[500] lg:hidden">
          <div className="absolute inset-0 bg-black/80 backdrop-blur-sm" onClick={() => setIsMobileMenuOpen(false)}></div>
          <div className="absolute top-0 left-0 w-[280px] h-full bg-[var(--theme-bg-dark)] p-6 shadow-2xl flex flex-col transition-colors duration-500">
            <div className="flex justify-between items-center mb-10">
              <span className="text-white text-xs font-bold tracking-[0.3em]">MENU</span>
              <button onClick={() => setIsMobileMenuOpen(false)}><X size={20} color="white" /></button>
            </div>
            <div className="flex flex-col gap-6">
              {['Speakers', 'Headphones', 'Smartwatches'].map(link => (
                <Link key={link} to="/shop" className="text-zinc-400 text-xs uppercase tracking-widest border-b border-white/10 pb-4" onClick={() => setIsMobileMenuOpen(false)}>{link}</Link>
              ))}
              {user ? (
                <button onClick={handleLogout} className="text-red-500 text-xs font-bold uppercase tracking-widest text-left">Logout</button>
              ) : (
