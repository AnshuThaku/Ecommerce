...
// Canvas environment ke liye custom AOS style - MORE INTENSE ANIMATION
const injectAOSStyles = () => {
  if (!document.getElementById('custom-aos-styles')) {
    ...
    style.innerHTML = `
      [data-aos="fade-up"] {
        opacity: 0;
        /* Animation ko slow aur smooth banaya gaya hai */
...
const getAllImages = (product) => {
  ...
  // Pehle main product images collect karein
  ...
  // Phir SAARE variants ki images collect karein (pehle sirf variant 0 utha raha tha)
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className={`relative bg-white overflow-y-auto transition-all duration-500 ease-in-out hide-scroll ${expand ? 'w-full h-[100vh] rounded-none shadow-none px-4 md:px-10' : 'w-full max-w-[95%] md:max-w-[1100px] max-h-[90vh] rounded-[24px] shadow-2xl px-4'
          }`}
            ...
            <div className="border-t border-gray-100 bg-white pt-10 px-4">
              <div className="flex gap-2 md:gap-4 mb-8 max-w-2xl mx-auto overflow-x-auto pb-2 justify-center px-2">
                <button
                  ...
                  className={`flex-1 min-w-[130px] md:min-w-[150px] flex items-center justify-center gap-2 md:gap-3 p-2 md:p-3 rounded-xl border transition-all ${activeDetailTab === 'overview' ? 'bg-[#0f0f0f] border-[#d3b574] text-white' : 'bg-[#f9f9f9] border-gray-100'
                    }`}
                >
                ...
                <button
                  ...
                  className={`flex-1 min-w-[130px] md:min-w-[150px] flex items-center justify-center gap-2 md:gap-3 p-2 md:p-3 rounded-xl border transition-all ${activeDetailTab === 'specs' ? 'bg-[#0f0f0f] border-[#d3b574] text-white' : 'bg-[#f9f9f9] border-gray-100'
                    }`}
                >
...
const ProductCard = ({ product, onQuickView }) => {
  ...
  useEffect(() => {
    ...
    if (cardImages.length > 1) {
      ...
      }, 3000); // Har 1 second mein saari images change hongi
    }
  ...
  return (
    ...
    >
      <div className="relative h-[350px] sm:h-[420px] bg-gray-100 overflow-hidden rounded-2xl cursor-pointer w-full">
        ...
        <div className="absolute inset-0 w-full h-full p-4 sm:p-6 transition-transform duration-1000 group-hover:scale-105">
          <img 
            ...
            alt={product.name} 
            className="w-full h-full object-contain mix-blend-multiply transition-opacity duration-700 ease-in-out" 
          />
        </div>
...
export default function ProductGrid({ title, subtitle, products }) {
  ...
  // Custom AOS observer (Animation repeat logic add kiya gaya hai)
  useEffect(() => {
    injectAOSStyles();
    const observer = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          // Screen mein aate hi animate karega
          entry.target.classList.add('aos-animate');
        } else {
          // Jaise hi screen se baahar jayega, animation reset ho jayega
          entry.target.classList.remove('aos-animate');
        }
      });
    }, { threshold: 0.15 }); // Thoda jaldi/der se trigger karne ke liye threshold set kiya hai
      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8 sm:mb-12">
          <div className="flex items-center space-x-4 mb-3 sm:mb-4">
            <div className="w-8 sm:w-10 h-[1px] bg-[#d3b574]"></div>
            <span className="text-[9px] sm:text-[10px] uppercase tracking-[0.4em] font-black text-[#d3b574]">Exclusively Curated</span>
          </div>
        ...
        <div className="flex flex-wrap justify-center gap-4 sm:gap-6">
          {products.map((p) => (
            <div key={p._id || Math.random()} className="w-full sm:w-[340px] max-w-[340px]">
  ...
  return (
    <section className="bg-white font-sans selection:bg-[#d3b574] selection:text-black overflow-hidden py-12 sm:py-20">
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-wrap justify-center gap-4 sm:gap-6">
          {products.map((product) => (
            <div key={product._id || Math.random()} className="w-full sm:w-[340px] max-w-[340px]">
