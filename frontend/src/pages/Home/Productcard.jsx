...
            // Agar backend relatedProducts bhej raha hai toh use karo, nahi toh local filter
            if (data.relatedProducts && data.relatedProducts.length > 0) {
                setSimilarProducts(data.relatedProducts.slice(0, 4));
            } else {
...
  // --- 2. HISTORY TRACKING (Trains the Recommendation Engine) ---
  useEffect(() => {
...
          }
          // Backend me view history add karega, isse home page par recommendations automatically aayenge
...
      trackProductView();
    }
  }, [fullProduct?._id, user]); // Dependency fixed to prevent infinite loops
...
                      isDealActive ? 'bg-red-600 text-white hover:bg-red-700' : 'bg-black text-white hover:bg-[#d3b574] hover:text-black'
                    }`}
